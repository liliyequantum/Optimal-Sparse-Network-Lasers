function [sol, t_cell, E_cell, N_cell, Iss_cell, Nss_cell] = lk_dde23_noise_specified(max_step, ...
...
if init_state_noise_switch == 0
    ...
    history = @(t) [abs_E0 * cos(params.W * t).*(1+params.sigma_init.*randn(params.M,1));
                           abs_E0 * sin(params.W * t).*(1+params.sigma_init.*randn(params.M,1));
                           max(steady_N0*ones(params.M,1).*(1+params.sigma_init.*randn(params.M,1)),0)];
end
