...
data = load(['./data/update_R2_freq_diode_data_d_',num2str(d_array(2)-d_array(1)),'_',num2str(d_array(end)),...
    '_M_',num2str(M),'_num_rng_w_',num2str(num_rng_w),'_std_w_',num2str(std_w),'.mat']);
...
varphi_mean_array = data.varphi_mean_array;
varphi_std_array = data.varphi_std_array;
...
for didx = 1:length(d_array)
    for i = 1:M
        x = d_array(didx);
        y = mod(varphi_mean_array(didx,i)+pi,2*pi)-pi;
        err = mod(varphi_std_array(didx,i)+pi,2*pi)-pi;
...
yticklabels({'-\pi', '0','\pi'})
xlabel('$\kappa~(ns^{-1})$','Interpreter','latex')
ylabel('$\varphi_{\textnormal{win}}$','Interpreter','latex')
set(gca, 'fontsize', 20, 'linewidth', 1.5)
box off
exportgraphics(gcf, 'figure_without_margins.png', 'Resolution', 300);
