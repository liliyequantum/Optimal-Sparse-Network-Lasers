...
intensity_fluc_std_array = zeros(num_d,M);
K_cell = cell(num_d, 1);
tic
for didx = 1:num_d
    % didx = 9;%num_d;
    ...
    for Midx = 1:M
        ...
        I = abs(E_sol(Midx,:)).^2;
        intensity_fluc_std_array(didx,Midx) = std((I - mean(I))/mean(I));
        % std((I - mean(I))/mean(I)).^2 - mean((I - mean(I)).^2/mean(I).^2)
...
save(['./data/update_R2_freq_diode_data_d_',num2str(d_array(2)-d_array(1)),'_',num2str(d_array(end)),'_M_',num2str(M),'_num_rng_w_', num2str(num_rng_w),...
    '_std_w_',num2str(std_w),'_1.mat'],'-v7.3')
% 
% end
