...
set(gca, 'FontSize', 20,'TickDir','out','linewidth',1.5,'YColor', purple);
xlim([0, length(k_i)+1]);
xticks([1, 6, 12, 18, 24]);
...
yyaxis right;
plot(1:M, abs(detuning_abs_mean), 'o-', 'color', [red_base 0.5], 'LineWidth', 1.5); hold on;
errorbar(1:M, abs(detuning_abs_mean), detuning_abs_std, 'color', [red_base 0.5], 'LineWidth', 1.2);
grid on;
ylabel('$|\langle\tilde{\Delta}_i\rangle|$ (rad/ns)','Interpreter','latex')
set(gca, 'FontSize', 20, 'TickDir','out','linewidth',1.5,'YColor', red_base);
