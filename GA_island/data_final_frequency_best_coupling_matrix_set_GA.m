...
num_rng_GA = 0;
data = load(['bestAdjmatrix_GA_',num2str(num_rng_GA),'.mat']);
bestAdjMatrix = data.bestAdjMatrix;
best_S = data.best_S;
...
freq_E_win_mean = zeros(M,length(best_S));
freq_E_win_std = zeros(M, length(best_S));

for cidx = 1:length(best_S)
    coupling_matrix = bestAdjMatrix{cidx};
    tic
    [sol, t_sol, ~, ~, ~, ~, ~, ave_S] = ...
    ...
    disp(['progress: ',num2str(cidx/length(best_S))])
end

save(['data_final_frequency_best_coupling_matrix_set_GA_',num2str(num_rng_GA),'.mat'],...
