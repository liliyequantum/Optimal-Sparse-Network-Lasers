function GA_island_pump(num_rng_ga,pump_factor)
    ...
    for i = 1:numIslands
        ...
        fitnessScores{i} = evaluateFitness(islands{i}, detuning, pump_factor);
    ...
    for gen = 1:numGenerations
        ...
        parfor i = 1:numIslands
            ...
            fitnessScores_offspring = evaluateFitness(offspring, detuning, pump_factor);
    ...
    save(['./data_coupling_matrix_set_GA/updated_GA_island_pump_',...
        num2str(pump_factor),'_num_rng_',num2str(num_rng_ga),'.mat'],'-v7.3');
...
function fitnessScores = evaluateFitness(population, detuning, pump_factor)
    % Evaluate fitness for each individual
    fitnessScores = zeros(length(population), 1);
    for i = 1:length(population)
        adjMatrix = population{i};
        fitnessScores(i) = calculateSpatialCoherence_pump(detuning, adjMatrix,pump_factor);
    end
end
