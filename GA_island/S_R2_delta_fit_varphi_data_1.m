...
intensity_mean_array = zeros(num_wrng,M);
intensity_std_array = zeros(num_wrng,M);
...
for rngwidx = 1:num_wrng
    ...
    for Midx = 1:M
        ...
        intensity_mean_array(rngwidx,Midx) = mean(abs(E_sol(Midx,:)).^2/Iss);
        intensity_std_array(rngwidx,Midx) = std(abs(E_sol(Midx,:)).^2/Iss);
    end
...
save('S_R2_delta_fit_varphi_data_1.mat')