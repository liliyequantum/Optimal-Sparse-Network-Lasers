...
num_rng_GA = 0;
data = load(['worstAdjmatrix_GA_',num2str(num_rng_GA),'.mat']);
worstAdjMatrix = data.worstAdjMatrix;
worst_S = data.worst_S;
...
freq_E_win_mean = zeros(M,length(worst_S));
freq_E_win_std = zeros(M, length(worst_S));

for cidx = 1:length(worst_S)
    coupling_matrix = worstAdjMatrix{cidx};
    tic
    [sol, t_sol, ~, ~, ~, ~, ~, ave_S] = ...
    ...
    disp(['progress: ',num2str(cidx/length(worst_S))])
end

save(['data_final_frequency_worst_coupling_matrix_set_GA_',num2str(num_rng_GA),'.mat'],...
    'freq_E_win_mean','freq_E_win_std')
