...
for cidx = 1:length(connectivity_array)
    if pump_factor == 4
        data_file = ['./data/diode_data_connectivity_',num2str(connectivity_array(cidx)),'_M_',num2str(M),'.mat'];
    else
    ...
    ave_S_cell{cidx} = temp_data.ave_S_array;
    max_S(cidx) = max(temp_data.ave_S_array);
    mean_S(cidx) = mean(temp_data.ave_S_array);
end
...
exportgraphics(gcf, 'sparse_sync_plot.png');