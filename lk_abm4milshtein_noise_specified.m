function [t_cell, E_cell, N_cell, Iss_cell, Nss_cell] = lk_abm4milshtein_noise_specified(params, h_abm,...
...
if init_state_noise_switch == 0
    delay_buffer = [abs_E0 .* cos(params.W .* t_delay_array);
                           abs_E0 .* sin(params.W .* t_delay_array);
                           steady_N0.*ones(params.M,buffer_length)];
else
    delay_buffer = [abs_E0 .* cos(params.W .* t_delay_array).*(1+params.sigma_init.*randn(params.M,buffer_length));
                           abs_E0 .* sin(params.W .* t_delay_array).*(1+params.sigma_init.*randn(params.M,buffer_length));
                           max(steady_N0*ones(params.M,buffer_length).*(1+params.sigma_init.*randn(params.M,buffer_length)),0)];
end
